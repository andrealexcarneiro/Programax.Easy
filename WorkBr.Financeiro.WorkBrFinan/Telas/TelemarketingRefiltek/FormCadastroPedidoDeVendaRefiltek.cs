...
{
    public partial class FormCadastroPedidoDeVendaRefiltek : FormularioPadrao
    {
        ...
        public FormCadastroPedidoDeVendaRefiltek(int numeroPedido = 0, int CodigoCliente = 0)
        {
            InitializeComponent();
            ...
            if (numeroPedido != 0)
            {
                ...
                txtIdCliente.Text = CodigoCliente.ToString();
        ...
        private void PreenchaProduto(Produto produto, bool exibirMensagemDeNaoEncontrado = false)
        {
            ...
            if (produto != null)
            {
                ...
                itensDisponiveis = produto.FormacaoPreco.Estoque - produto.FormacaoPreco.EstoqueReservado;
        ...
        private void InsiraOuAtualizeItemPedido()
        {
            ...
            if((EnumTipoPedidoDeVenda)cboTipoDocumento.EditValue != EnumTipoPedidoDeVenda.ORCAMENTO)
            {
                ...
                if (servicoPedido.VerifiqueItemQuantidadeEstoqueNegativo(txtQuantidadeProduto.Text.ToDouble(), _produtoEmEdicao, false))
                {
                    MessageBox.Show("O estoque do seguinte item: " + _produtoEmEdicao.Id + " - " + _produtoEmEdicao.DadosGerais.Descricao
                                    + ". Pode estar zerado ou a quantidade requerida não está disponível!", 
            ...
            Action actionInserirItem = () =>
            {
                ...
                if (ValideSeItemEstahEmBaixa(pedidoDeVenda.StatusPedidoVenda, itemPedidoVenda))
                {
                    return;
                }
        ...
        public void PreenchaPedidoDeVenda(PedidoDeVenda pedidoDeVenda, bool exibirMensagemDeNaoEncontrado = false)
        {
            ...
            {
                List<ItemPedidoAuxiliar> listaitempedido = new List<ItemPedidoAuxiliar>();
                double ValorTotal = 0;
                MySqlCommand command = new MySqlCommand("ConsultaItensRefiltek", conn);
                command.CommandType = CommandType.StoredProcedure;


                command.Parameters.AddWithValue("varPedido", txtId.Text);
                ...
                while (returnValue.Read())
                {
                    ...
                    itemPedidoAuxiliar.PedidoId = returnValue["peditem_id"].ToString();
                    itemPedidoAuxiliar.CodProduto = returnValue["peditem_produto_id"].ToString();
                    itemPedidoAuxiliar.Descricao = returnValue["prod_descricao"].ToString();
                    itemPedidoAuxiliar.Valorunitario = returnValue["peditem_valor_unitario"].ToString();
                    itemPedidoAuxiliar.Quantidade = returnValue["PEDITEM_QUANTIDADE"].ToString();
                    itemPedidoAuxiliar.Total = returnValue["peditem_valor_total"].ToString();
                    itemPedidoAuxiliar.Desconto = returnValue["peditem_total_desconto"].ToString();
                    itemPedidoAuxiliar.Unidade = returnValue["UND_ABREVIACAO"].ToString();
                    listaitempedido.Add(itemPedidoAuxiliar);
                    ValorTotal += itemPedidoAuxiliar.Total.ToDouble();
                    //}

                }
                gcItens.DataSource = listaitempedido;
                gcItens.RefreshDataSource();
                txtValorTotalProduto.Text = ValorTotal.ToString();
            }
            ...
            using (var conn = new MySqlConnection(ConectionStringII))
            {
                ...
                command.Parameters.AddWithValue("varCodigo", txtIdCliente.Text);
                ...
                while (returnValue.Read())
                {
                    txtNomeCliente.Text = returnValue["pes_fantasia"].ToString();
                    txtRazaoSocial.Text = returnValue["pes_razao"].ToString();
            ...
            using (var conn = new MySqlConnection(ConectionStringII))
            {
                ...
                while (returnValue.Read())
                {
                   ...
                   //cboTipoDocumento.EditValue = 2;

                   // txtCpfCnpj.Text = returnValue["peditem_valor_unitario"].ToString();
                   // cboTipoEndereco.EditValue = returnValue["endpes_tipo_endereco"].ToString();
                    txtCepEndereco.Text = returnValue["PEDIDO_CEP_ENDERECO"].ToString();
                    cboCidade.EditValue = returnValue["PEDIDO_CIDADE_ID_ENDERECO"].ToString();
                    txtBairro.Text = returnValue["PEDIDO_BAIRRO_ENDERECO"].ToString();
                    txtRua.Text = returnValue["PEDIDO_RUA_ENDERECO"].ToString();
                    //cboIndicadores.EditValue = returnValue["pedido_indicador_id"].ToString();
        ...
        {
            int _idCliente = 0;
            FormVendaRapida formVendaRapida = new FormVendaRapida(txtId.Text.ToInt());
