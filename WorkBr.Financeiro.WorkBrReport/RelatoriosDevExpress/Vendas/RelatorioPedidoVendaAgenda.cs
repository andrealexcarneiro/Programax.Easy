...
{
    public partial class RelatorioPedidoVendaAgenda : RelatorioBaseDev
    {
        #region " VARIÁVEIS PRIVADAS "

        private int _numeroPedido;
        private Parametros _parametros;
        private EnumTipoEndereco? _tipoEndereco;

        #endregion

        #region " CONSTRUTOR "

        public RelatorioPedidoVendaAgenda(int numeroPedido, EnumTipoEndereco? tipoEndereco = null)
        {
            InitializeComponent();
        ...
        protected override void CarregueDadosRelatorio()
        {
            ...
            if (pedido.EnderecoPedidoDeVenda != null)
            {
                ...
                //{
                    //if (pedido.Cliente != null)
                    //{
                    //    var enderencoAgenda = new ServicoPessoa()
                    //     .ConsulteClienteAtivo(pedido.Cliente.Id).ListaDeEnderecos.FirstOrDefault(x => x.TipoEndereco == _tipoEndereco);

                    //    if(enderencoAgenda != null)
                    //    {
                    //        pedidoDeVendaRelatorio.Endereco = enderencoAgenda.Rua;
                    //        pedidoDeVendaRelatorio.Numero = enderencoAgenda.Numero;
                    //        pedidoDeVendaRelatorio.Complemento = enderencoAgenda.Complemento;
                    //        pedidoDeVendaRelatorio.Bairro = enderencoAgenda.Bairro;
                    //        pedidoDeVendaRelatorio.Cidade = enderencoAgenda.Cidade.Descricao;
                    //        pedidoDeVendaRelatorio.Estado = enderencoAgenda.Cidade.Estado != null ? pedido.EnderecoPedidoDeVenda.Cidade.Estado.UF : string.Empty;
                    //        pedidoDeVendaRelatorio.Cep = enderencoAgenda.CEP;
                    //    }
                    //}
                //}
                //else
                {
                    var endereco = pedido.Cliente.ListaDeEnderecos.FirstOrDefault(end => end.TipoEndereco == _tipoEndereco);
            ...
            pedido = servicoPedidoDeVenda.Consulte(pedido.Id);
            ...
            //Agenda            
            var agenda = new ServicoRoteiro().ConsultePorPedido(pedido.Id);

            if (agenda != null)
            {
                lblDataAgenda.Text = agenda.DataElaboracao.ToString("dd/MM/yyyy");
                lblLigarAntes.Text = agenda.Observacao;
                lblPeriodo.Text = agenda.Periodo.Descricao();
                lblServico.Text = agenda.TipoServico.Descricao();

                var roteiro = new ServicoRoteirizacao().Consulte(agenda.RoteirizacaoId.ToInt());

                if (roteiro != null)
                    lblTecnico.Text = roteiro.PessoaFuncionario.DadosGerais.Razao;

                lblObsAgenda.Text = agenda.DetalheServico;
                lblStatus.Text = agenda.Status.Descricao();
            }
