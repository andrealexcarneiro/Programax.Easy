...
{
    public partial class RelatorioVendasClientes : RelatorioBasePaisagem
    {
        ...
        public RelatorioVendasClientes(Pessoa cliente,
            ...
            foreach (var venda in listaVWVenda)
                {
                    ...
                    //VendasClienteRelatorio.CodProduto = pedidoDeVendaItem.ListaItens(venda.Id)
                        ...
                        VendasClienteRelatorio.Bairro = venda.Bairro;
                        ...
                        VendasClienteRelatorio.ValorPedido = venda.ValorTotal.ToString();
            ...
            for (int i = 0; i < _listaFormasPagamento.Count; i++)
            {
                ...
                foreach (var venda in listaVWVenda)
                {
                    ...
                    foreach (var item in pedidoDeVendaItem.ListaItens)
                    {
                        ...
                        itemVenda.Desconto = item.TotalDesconto.ToString("#,###,##0.00");
                        itemVenda.ValorTotal = (item.Quantidade * item.ValorUnitario - item.TotalDesconto).ToString("#,###,##0.00");
                        QReserva = item.Produto.FormacaoPreco.EstoqueReservado.ToDouble();
                        if (QReserva > 0)
                        {
                            itemVenda.Reserva = item.Quantidade.ToString("#,###,##0.00");
                        }
                        else
                        {
                            itemVenda.Reserva = "0,00";
                        }
