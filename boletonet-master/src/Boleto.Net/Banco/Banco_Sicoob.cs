...
namespace BoletoNet
{
    ...
    internal class Banco_Sicoob : AbstractBanco, IBanco
    {
        ...
        public override void FormataLinhaDigitavel(Boleto boleto)
        {
            ...
            {
                //Calculando indice
                temp = (Convert.ToInt32(campo1.Substring(i, 1)) * Convert.ToInt32(indice.Substring(i + 1, 1)));
        ...
        private string GerarHeaderRemessaCNAB240(int numeroConvenio, Cedente cedente, int numeroArquivoRemessa)
        {
            ...
            try
            {
                ...
                header += new string('0', 1); //Posição 072 a 72     Dígito Verificador da Ag/Conta: Brancos ***Novo Layout 08/07/2019 com Zero
        ...
        private string NossoNumeroFormatado( Boleto boleto )
        {
            ...
            retorno = retorno + Utils.FormatCode(boleto.TipoModalidade.ToString(), "0", 2, true); // modalidade - 2 posicoes
        ...
        public override string GerarDetalheSegmentoPRemessa(Boleto boleto, int numeroRegistro, string numeroConvenio)
        {
            try
            {
                ...
                detalhe += Utils.FormatCode(boleto.Remessa.NumeroLote.ToString(), "0", 4, true); //Posição 004 a 007   Número Sequencial
                ...
                detalhe += Utils.FormatCode("0", 5); //***Alterado 04/07/2019 //Utils.FormatCode(boleto.ContaBancaria.Agencia, 5);//Posição 101 a 105     Agência Encarregada da Cobrança: "00000"
                ...
                detalhe += Utils.FormatCode(boleto.DataVencimento.AddDays(1).ToString("ddMMyyyy"), 8);  //Posição 119 a 126  - Data do Juros de Mora: preencher com a Data de Vencimento do Título
                ...
                detalhe += "0"; //Posição 118  - Código do desconto
                
                //***Com o código do desconto é zero, ou seja, não está trabalhando com desconto, na linha ativa abaixo, será formatado com zeros
                //detalhe += Utils.FormatCode(boleto.DataDesconto.ToString("ddMMyyyy"), 8); //Posição 143 a 150  - Data do Desconto 1
                detalhe += Utils.FormatCode("0", 8); //Posição 143 a 150  - Data do Desconto 1
                //*****
                ...
                foreach (IInstrucao instrucao in boleto.Instrucoes)
                {
                    switch ((EnumInstrucoes_Sicoob)instrucao.Codigo)
                    {
                        ...
                        case EnumInstrucoes_Sicoob.Protestar3DiasUteis:
                            vInstrucao1 = Utils.FitStringLength("3", 2, 2, '0', 0, true, true, true);
                            break;

                        case EnumInstrucoes_Sicoob.Protestar4DiasUteis:
                            vInstrucao1 = Utils.FitStringLength("4", 2, 2, '0', 0, true, true, true);
                            break;

                        case EnumInstrucoes_Sicoob.Protestar5DiasUteis:
                            vInstrucao1 = Utils.FitStringLength("5", 2, 2, '0', 0, true, true, true);
                            break;

                        case EnumInstrucoes_Sicoob.Protestar10DiasUteis:
                            vInstrucao1 = Utils.FitStringLength("10", 2, 2, '0', 0, true, true, true);
                            break;

                        case EnumInstrucoes_Sicoob.Protestar15DiasUteis:
                            vInstrucao1 = Utils.FitStringLength("15", 2, 2, '0', 0, true, true, true);
                            break;

                        case EnumInstrucoes_Sicoob.Protestar20DiasUteis:
                            vInstrucao1 = Utils.FitStringLength("20", 2, 2, '0', 0, true, true, true);
                            break;
                    }
                }
                #endregion
                ...
                detalhe += new string(' ', 3) ;//***Alterado 04/07/2019//Utils.FormatCode("0", 3);     //Posição 225 A 227  - Número de Dias para Baixa/Devolução: Brancos
        ...
        public override string GerarDetalheSegmentoQRemessa(Boleto boleto, int numeroRegistro, TipoArquivo tipoArquivo)
        {
            ...
            {                
                ...
                detalhe += "0";//detalhe += (boleto.Sacado.CPFCNPJ.Length == 11 ? "1" : "2");           // Tipo de Inscrição Sacador avalista (Não tendo avalista será "0")
                detalhe += Utils.FormatCode("0", "0", 15); //detalhe += Utils.FormatCode(boleto.Sacado.CPFCNPJ, "0", 15);  // Número de Inscrição / Sacador avalista (Não tendo avalista será preenchido com zeros)
                detalhe += Utils.FormatCode("", " ", 40); //detalhe += Utils.FormatCode(boleto.Sacado.Nome, " ", 40);   // Nome / Sacador avalista (Não tendo avalista será preenchido com brancos)
        ...
        public override string GerarTrailerLoteRemessa(int numeroRegistro, int numeroLote=1)
        {
            try
            {
                string trailer = Utils.FormatCode(Codigo.ToString(), "0", 3, true); //Código do banco
                trailer += Utils.FormatCode(numeroLote.ToString(), "0", 4, true); //Posição Uso 4 a 7  -  Número Lote - Sequencial
                trailer += "5";
